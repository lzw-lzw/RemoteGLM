...
from finetune_RemoteGLM import FineTuneVisualGLMModel
...
def main():
    ...
    parser.add_argument("--prompt_zh", type=str, default="你好", help='Chinese prompt for the first round')
    ...
    if not args.english:
        print('欢迎使用 RemoteGLM 模型，输入图像URL或本地路径读图，继续输入内容对话，clear 重新开始，stop 终止程序')
    else:
        print('Welcome to RemoteGLM model. Enter an image URL or local file path to load an image. Continue inputting text to engage in a conversation. Type "clear" to start over, or "stop" to end the program.')
    with torch.no_grad():
        while True:
            history = None
            cache_image = None
            ...
            while True:
                ...
                print("RemoteGLM："+response.split(sep)[-1].strip())
                image_path = None
                if not args.english:
                    query = input("用户：")
                else:
